...
Сейчас у твоего персонажа {} здоровья. За каждую победу ты получишь дополнительно {}.
Таким образом, максимальное значение здоровье после испытания может быть {}.\n'''.format(
    colored('три битвы', 'blue'), colored('40 единиц', 'blue'), colored('20 единиц', 'blue'), colored('100 единиц', 'blue')
)
RULES_1 = '''{0}
Твоя цель — за {1} набрать такое количество очков урона своему противнику, которое попадёт в диапазон {2} от его изначального здоровья.
Здоровье противника каждый раз генерируется случайным образом в диапазоне {3} единиц.
{4} если противник получил 100 здоровья, то для победы необходимо за 5 атак нанести ему суммарный урон от 90 до 110.
...
{attack}'''.format(
    ...
    colored('± 10', 'blue', attrs=['bold']),
    colored('от 80 до 120', 'blue', attrs=['bold']),
    colored('Пример:', 'black', attrs=['bold']),
    attack=ATTACK
)
RULES_2 = '''Сперва ты можешь немного попрактиковаться - тренировочные бои ни на что не влияют.
Когда посчитаешь, что готов, переходи к трём сражениям на результат.
Чем больше побед ты в них одержишь, тем больше очков здоровья получишь.
Приступай, как только будешь готов.'''
TRAIN = (
    '-> Как-то боязно - хочу ещё потренить (введи {})\n'
    '-> Мортал комбат!!! (введи {})\n>>> '.format(
        colored('train', 'blue', attrs=['bold']), colored('fight', 'blue', attrs=['bold'])
    )
)
...
def run_game(fight: bool = False) -> Union[bool, int]:
    ...
    if 0 <= point_difference <= 10:
        ...
        winning = 0
    if not fight:
        train_quit = {'train': True, 'fight': False}
...
def start() -> tuple[int, int]:
    ...
    health = 40 + wins * 20  # минимум 40 ОЗ и по 20 ОЗ за каждую победу
    return wins, health
